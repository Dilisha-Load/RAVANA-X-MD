...
async function downloadSession(sessdata, df) {
  ...
  for (let i = 0; i < dbUrls.length; i++) {
    ...
    console.log(`📥 Downloading session from Ravana-DB...`);
...
if (!fs.existsSync(df) && config.SESSION_ID) {
    const sessdata = config.SESSION_ID.replace("RAVANA-UID&", "");
    (async () => {
...
async function downloadExternalPlugins() {
    try {
        ...
        if (megaUrl && megaUrl.includes("mega.nz")) {
            console.log("📥 Extracting Files For Ravana...");
            ...
            console.log("✅ Files Extract Successfily!");
        }
    } catch (err) {
    ...
    if (connection === 'close') {
        ...
        setTimeout(async () => {
            try {
                ...
                let captionText = '✅ RAVANA connected successfully!';
                try {
                    const response = await axios.get('https://raw.githubusercontent.com/RAVANA-PRODUCT/database/refs/heads/main/main_var.json');
...
const leaveMsg = config.LEAVE_MSG;
  ...
  const can = `
*🎯 Ｒᴀᴠᴀɴᴀ－Ｘ－Ｍᴅ SETTINGS CONNECTED 🎯*
...
await connectdb()
await updb()		
console.log('RAVANA SETTINGS DL CONNECTED ✅')
...
const isBanvisper = banGroups
  ...
  .includes(from);   
const SUDO = config.SUDO; // eg: [ '94754871798@s.whatsapp.net', '120363405532194833@g.us' ]
...
if ( isBanUser ) {
	...
	return await conn.sendMessage(from, { text: "*You are banned by RAVANA GANG ❌*" })
}
...
if(!isOwner) {	
    ...
    //await conn.sendMessage(from, { audio: fs.readFileSync("./" + type.ext), mimetype:  originalMessage.message.audioMessage.mimetype, fileName:  `${m.id}.mp3` })	
     const sdata = await conn.sendMessage(delfrom,{sticker: fs.readFileSync("./" + type.ext) ,package: 'Ｒᴀᴠᴀɴᴀ－Ｘ－Ｍᴅ 🌟'})
    return await conn.sendMessage(delfrom, { text: `🚫 *This message was deleted !!*\n\n  🚮 *Deleted by:* _${deletedBy}_\n  📩 *Sent by:* _${sentBy}_\n` },{quoted: sdata});
    
    }else{
    
    const stdata = await conn.sendMessage(delfrom,{sticker: fs.readFileSync("./" + type.ext) ,package: 'Ｒᴀᴠᴀɴᴀ－Ｘ－Ｍᴅ 🌟'})
    return await conn.sendMessage(delfrom, { text: `🚫 *This message was deleted !!*\n\n  🚮 *Deleted by:* _${deletedBy}_\n  📩 *Sent by:* _${sentBy}_\n` },{quoted: stdata});
...
app.get("/", (req, res) => {
  res.send("🎯 RAVANA SETTINGS Working successfully!");
});
app.listen(port, () => console.log(`Ravana-X-Md Server listening on port http://localhost:${port}`));
setTimeout(() => {
connectToWA()
}, 3000);
